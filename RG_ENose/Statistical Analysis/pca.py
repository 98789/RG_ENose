...
def pca(data, scl, n_components, plot=1):
    ...
    var100 = pca.explained_variance_ratios * 100
    ssq = zip(range(1, m + 1), pca.explained_variance_, var100, numpy.cumsum(var100))
    ...
    tsqs = numpy.sum(numpy.dot(scores, numpy.linalg.inv(numpy.diag(ssq ** 0.5))) ** 2,
           1) if n_components > 1 else scores ** 2 / ssq[0, 1]

    return [scores, loads, ssq, res, q, tsq, tsqs]
